...
### Voice transcription (handled by the companion **Rankore Voice** bot — must also be invited to the server)
- `!transcribe_join [lang=en|it|es|fr]`: voice bot joins your current voice channel and starts recording. Optional `lang=` pin (default: auto-detect). Supported languages: English, Italian, Spanish, French.
- `!transcribe_leave`: stops recording and replies with the transcript `.txt` attached.
- `!transcribe_status`: shows current transcription session info.
